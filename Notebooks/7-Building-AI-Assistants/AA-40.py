...
# -----------------------------------------------------------------------------
# Import warnings and ignore that Pydantic .json() deprecation warning for Anthropic
# -----------------------------------------------------------------------------
import warnings
...
def openai_chat_completion(messages, model, temperature, top_p, max_tokens, api_key):
    """
    Manually map SystemMessage/HumanMessage to the correct role for OpenAI.
    """
...
# -----------------------------------------------------------------------------
# Unified Follow-Up Queries for OpenAI & Anthropic
# -----------------------------------------------------------------------------
import re
...
def generate_follow_up_queries(
    ...
    """
    Generates EXACTLY three short follow-up questions, each line starting with '1. ', '2. ', '3. '.
    If the LLM strays, we filter & truncate lines so we end up with precisely three enumerated lines.
    """
    final_max_tokens = llm_params.get("max_tokens", 512)

    # Tighter instructions to reduce extraneous text:
    system_text = (
        f"You have ~{final_max_tokens} tokens. "
        "Please provide EXACTLY three short follow-up questions. "
        "Number them '1. ', '2. ', '3. ' with no extra introduction or text. "
        "No bullet points or other numbering style. Avoid abrupt cutoffs."
    )

    followup_prompt = (
        f"User Query: {prompt}\n"
        f"Response:\n{response}\n\n"
        "Your output MUST be exactly three lines, each starting with '1. ', '2. ', '3. '. "
        "No other lines are allowed. Begin now:"
    )

    # We'll create two messages:
    messages = [
    ...
    try:
        if provider == "openai":
            ...
            anthro_client = ChatAnthropic(
                model=model,
                anthropic_api_key=api_key,
                temperature=llm_params.get("temperature", 0.5)
            )
            resp = anthro_client.invoke(
                input=messages,
                max_tokens=final_max_tokens
            )
            text = resp.content
        else:
            return [f"Error: Unknown provider '{provider}' for follow-ups"]

        # Post-process the text to keep exactly three lines starting with '1. ', '2. ', '3. '
        lines = [line.strip() for line in text.split("\n") if line.strip()]
        pattern = re.compile(r"^[123]\.\s")
        filtered = [ln for ln in lines if pattern.match(ln)]
        filtered = filtered[:3]  # keep up to 3

        # If fewer than 3 lines remain, fill them with placeholders
        while len(filtered) < 3:
            filtered.append("Error: The model did not provide all three lines properly.")
...
# -----------------------------------------------------------------------------
# Anthropic Token Counting Helpers
# -----------------------------------------------------------------------------
...
def anthropic_count_tokens(model: str, messages, api_key: str):
    ...
    try:
        ...
        raw = resp.json()  # Might be dict or str
...
response_length = st.sidebar.radio(
    ...
    help="Select a preset for response detail."
)
...
follow_up_enabled = st.sidebar.checkbox(
    "Enable Follow-up Queries",
    help="Generate follow-up questions after the response."
)
...
# -----------------------------------------------------------------------------
    ...
    model_max_token_limit = 8192  # Example fallback
...
# Dynamic slider for max output tokens
...
if submit_button and user_input:
    ...
    try:
        ...
        if llm_provider == "OpenAI Dynamic":
            ...
            # If you had matched_models, you can do
            # matched_obj = next((m for m in matched_models if m["dynamic_id"] == dyn_id), None)
            final_model = dyn_id  # fallback if unmatched
    ...
    if follow_up_enabled:
        if llm_provider == "OpenAI Dynamic":
            ...
            st.subheader("Follow-up Questions (OpenAI):")
            ...
            st.subheader("Follow-up Questions (Anthropic):")
            for idx, q in enumerate(follow_ups, start=1):
                st.markdown(f"**{idx}.** {q}")
        # Else, skip for Google / XAI if not implemented
