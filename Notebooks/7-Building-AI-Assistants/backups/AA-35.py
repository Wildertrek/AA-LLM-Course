...
def openai_chat_completion(messages, model, temperature, top_p, max_tokens, api_key):
    """
    Fix for the 'SystemMessage' object has no attribute 'role' issue:
    we explicitly map SystemMessage -> system, HumanMessage -> user, etc.
    """
...
# Instead of a static list, let's also handle "Choose Embedding Model" with real metadata.
# We'll do that below. For now, keep the variable name for logic usage.
index_name = st.sidebar.text_input("Vector DB Index Name", value="my_vector_index", help="Vector DB identifier.")


# -----------------------------------------------------------------------------
# Dynamically Load OpenAI Embedding Models from Metadata
# -----------------------------------------------------------------------------
embedding_metadata_dict = load_openai_model_metadata_as_dict()

# Filter to find any that have the "Embeddings" endpoint supported
embedding_candidates = []
for model_id, info in embedding_metadata_dict.items():
    endpoints = info.get("endpoints", {})
    # If "Embeddings" is a key and not "Not supported", consider it an embedding model
    if "Embeddings" in endpoints and endpoints["Embeddings"] != "Not supported":
        embedding_candidates.append(model_id)

# Build user-friendly labels: "text-embedding-ada-002 => text-embedding-ada-002"
embedding_options = []
for ec in embedding_candidates:
    display_name = embedding_metadata_dict[ec].get("display_name", ec)
    embedding_options.append(f"{ec} => {display_name}")
...
if embedding_info:
    ...
    # Show e.g. context_window, pricing
    cwin = embedding_info.get("context_window", "N/A")
    st.sidebar.markdown(f"**Context Window:** {cwin}")
    st.sidebar.markdown(f"**Endpoints:** {embedding_info.get('endpoints', {})}")

    if st.sidebar.checkbox("Show Embedding Full Metadata"):
        st.sidebar.json(embedding_info)
else:
    st.sidebar.write("No metadata found for this embedding ID.")
...
if st.sidebar.button("Process Files", help="Embed and index uploaded files."):
    ...
    for uploaded_file in uploaded_files:
        ...
        # Insert your actual embedding or indexing logic here, using "embedding_id" if needed
        # For example, call an embed function with the chosen embedding model.
...
# Main Query Section
# -----------------------------------------------------------------------------
st.title("AI Assistant with Model Metadata (Embeddings & GPT)")
...
if submit_button and user_input:
    ...
    try:
        if llm_provider == "OpenAI Dynamic":
            # Let's do the matched snapshot approach for GPT Chat models
            metadata_dict = load_openai_model_metadata_as_dict()
            matched_models = get_matched_openai_models(openai_api_key, metadata_dict)

            # Suppose you stored the user's GPT choice in chosen_model_label from the sidebar
            # We'll do the same approach as before, or we can replicate the logic:
            # For demonstration, let's say we introduced a variable "chosen_gpt_model_label" 
            # If your code is already set up so the user picks that, do the same logic:
            # But to keep the example short, we'll just pick the first matched model:
            if matched_models:
                chosen_model_label = st.session_state.get("chosen_gpt_model_label", None)
                # If we don't have a separate GPT model selection, let's default to "gpt-3.5-turbo"
                if not chosen_model_label:
                    chosen_model_label = f"{matched_models[0]['dynamic_id']} => {matched_models[0]['display_name']}"
            else:
                chosen_model_label = "gpt-3.5-turbo => GPT-3.5 Turbo"
    ...
    if follow_up_enabled and (llm_provider == "OpenAI Dynamic"):
        # For follow-up queries, assume final_model is correct
        follow_ups = generate_follow_up_queries(user_input, response, llm_params, final_model, openai_api_key)
