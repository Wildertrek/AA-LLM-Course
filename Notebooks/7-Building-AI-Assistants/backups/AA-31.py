...
# Define provider selection before calling any provider-specific selectors
...
# Only show the OpenAI selector if that provider is chosen
...
def openai_chat_completion(messages, model, temperature, top_p, max_tokens, api_key):
    ...
    # Convert messages to dictionaries; assign roles based on type.
    messages_payload = []
    for m in messages:
...
def generate_follow_up_queries(prompt: str, response: str, llm_params: dict, selected_model: str, api_key: str):
    """
    Generates follow-up questions using OpenAI's Chat Completion with the dynamically selected model.
    """
    follow_up_prompt = (
    ...
    try:
        ...
        system_text = f"You have ~{final_max_tokens} tokens. Provide exactly three short follow-up questions. Avoid abrupt cutoffs."
        ...
        follow_up_text = openai_client.invoke(input=messages, temperature=llm_params.get("temperature", 0.5), max_tokens=final_max_tokens).content
...
def save_chat_completion_log(completion_details, debug_headers, filename="chat_completions_log.json"):
    """
    Saves the chat completion details, debug headers, and token usage to a JSON log file.
    """
    log_entry = {
...
# Update slider: limit number of referenced responses to max 5
num_references = st.sidebar.slider("Web Search Number of Referenced Responses Desired", 1, 5, 5, help="Specify how many referenced responses you want to retrieve from web search.")
...
if submit_button and user_input:
    ...
    start_time = time.time()  # Start timing for main query
    try:
    ...
    elapsed_time = time.time() - start_time  # Calculate elapsed time for main query
    ...
    if follow_up_enabled:
        # For follow-up queries, use OpenAI Dynamic as the default
...
if web_search_button:
    if not user_input:
        ...
        search_start = time.time()  # Start timing for web search
