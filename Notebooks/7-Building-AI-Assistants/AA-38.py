...
# Main Query Section
# -----------------------------------------------------------------------------
st.title("AI Assistant with OpenAI / Anthropic & Dynamic Max Tokens")
...
if submit_button and user_input:
    ...
    try:
        ...
        if llm_provider == "OpenAI Dynamic":
            ...
            # Build messages in the same style
            messages = build_system_user_messages(user_input, final_max, system_persona, user_persona)

            # We'll call ChatAnthropic from langchain_anthropic
            dynamic_anthropic = ChatAnthropic(
                model=final_model,
                temperature=llm_params["temperature"],
                anthropic_api_key=anthropic_api_key
            )
            # For Anthropic, "max_tokens" param is slightly different
            # We'll pass final_max to "max_tokens_to_sample" in the .invoke
            # but we can do .invoke(input=messages, max_tokens=final_max)
            ...
            # e.g. final_model = selected_google_model
            pass
