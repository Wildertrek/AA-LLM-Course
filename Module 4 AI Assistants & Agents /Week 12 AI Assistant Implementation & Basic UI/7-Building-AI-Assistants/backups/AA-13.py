...
# -----------------------------------------------------------------------------
# Dynamic Model Listing Functions (for OpenAI, Anthropic, and Google via SDK)
# -----------------------------------------------------------------------------
...
def get_available_xai_models(api_key: str):
    """
    Retrieve available XAI models from the XAI API at https://api.x.ai/v1/models.
    """
...
# -----------------------------------------------------------------------------
# Layout: Two-Column Design (Left: Main Content, Right: File I/O & RAG Panel)
# -----------------------------------------------------------------------------
main_col, rag_col = st.columns([3, 1])

# Left Column: Main Query and LLM Response
with main_col:
    ...
    if llm_provider == "OpenAI Dynamic":
        ...
        selected_openai_model = st.selectbox("Select an OpenAI Chat Model", available_openai_models or ["gpt-4", "gpt-3.5-turbo"])
    if llm_provider == "Anthropic Dynamic":
        available_anthropic_models = get_available_anthropic_models(anthropic_api_key)
        selected_anthropic_model = st.selectbox("Select an Anthropic Chat Model", available_anthropic_models or ["claude-3-5-sonnet-20241022", "claude-2", "claude-instant"])
    if llm_provider == "Google Dynamic":
        available_google_models = get_available_google_models_via_sdk(gemini_api_key)
        selected_google_model = st.selectbox("Select a Google Model", available_google_models or ["gemini-2.0-flash-001"])
    if llm_provider == "XAI Dynamic":
        available_xai_models = get_available_xai_models(xai_api_key)
        selected_xai_model = st.selectbox("Select a XAI Model", available_xai_models or ["grok-2-latest"])
...
# -----------------------------------------------------------------------------
# Right Column: File I/O & RAG Pipeline Configuration Panel
# -----------------------------------------------------------------------------
with rag_col:
    ...
    st.markdown("### Vector Database Index Name")
    index_name = st.text_input("Index Name", value="my_vector_index")
    
    if st.button("Upload & Process Files"):
        st.info("Processing uploaded files for RAG pipeline...")
        for uploaded_file in uploaded_files:
            st.write(f"Processing file: {uploaded_file.name}")
            # Here, integrate your file processing, embedding generation, and indexing logic.
        st.success("Files processed and indexed successfully!")
    
    st.markdown("### Status Monitoring")
    status_placeholder = st.empty()
    status_placeholder.text("No current status.")
