...
def get_available_openai_models(api_key: str):
    ...
    try:
        ...
        if response.ok:
            ...
            # Filter for models starting with "gpt-"
...
def openai_chat_completion(messages, model, temperature, top_p, max_tokens, api_key):
    ...
    # Convert messages to dicts; assign roles based on instance type.
    messages_payload = []
    for m in messages:
...
def query_grok(prompt: str, model: str, llm_params: dict):
    try:
        # For Grok, use the selected model and pass in prompt
...
def generate_follow_up_queries(prompt: str, response: str, llm_params: dict):
    """
    Generates follow-up questions using the OpenAI Chat API (GPT-4o).
    """
    follow_up_prompt = (
...
if submit_button and user_input:
    ...
    try:
        ...
        if llm_provider == "OpenAI Dynamic":
            ...
            final_max = max_tokens_slider  # Use selected max tokens directly
